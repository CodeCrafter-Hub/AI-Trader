...
Start all MCP services: Math, Search, TradeTools, LocalPrices, LiveTradeTools, LiveMarketData
"""
...
class MCPServiceManager:
    def __init__(self):
        ...
        self.ports = {
            ...
            'live_trade': int(os.getenv('LIVE_TRADE_HTTP_PORT', '8004')),
            'live_market': int(os.getenv('LIVE_MARKET_HTTP_PORT', '8005'))
        }
        ...
        self.service_configs = {
            ...
            'live_trade': {
                'script': 'tool_live_trade.py',
                'name': 'LiveTradeTools',
                'port': self.ports['live_trade']
            },
            'live_market': {
                'script': 'tool_live_market.py',
                'name': 'LiveMarketData',
                'port': self.ports['live_market']
            }
        }
            ...
            while self.running:
                ...
                for service_id, service in self.services.items():
                    if service['process'].poll() is not None:
                        print(f"\n??  {service['name']} service stopped unexpectedly")
                        self.running = False
                        break
